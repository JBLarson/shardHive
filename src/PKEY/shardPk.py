...
def shardAndEncrypt(userId, pKey):
	...
	# create user tmp dir
	try:	os.system('mkdir ' + str(userId) + '/tmp')
	except Exception:	pass
	...
	for shard in pkShards:
		...
		jsonAddr = str(userId) + '/tmp/' + str(shardIndex) + '.json'
	...
	for shard in pkShards:
		...
		currentJsonAddr = str(userId) + '/' + str(shardIndex) + '.json'
		currentTmpJsonAddr = str(userId) + '/tmp/' + str(shardIndex) + '.json'
	...
	# remove tmp dir
	try:	os.system('rm -rf ' + str(userId) + '/tmp')
	except Exception:	pass
...
try:
	outputAddr = str(theUserId) + '/shardMap.json'
